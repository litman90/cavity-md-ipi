...
class PhotonDriverFabryPerot:
    ...
    def __init__(
        ...
        domega_x_cminv=100.0,
        domega_y_cminv=100.0,
        n_mode_x=4,
        n_mode_y=3,
        x_grid_1d=np.array([0.1, 0.5, 0.9]),
        y_grid_1d=np.array([0.1, 0.5]),
        ph_constraint="none",
        ph_rep="loose",
    ):
        """
    ...
    def init_fabry_perot_geometry(self, ph_constraint="none", ph_rep="loose"):
        ...
        if ph_constraint == "single_mode":
            ...
            self.domega_y = 0.0
            self.n_mode_y = 1
            self.y_grid_1d = np.array([0.0])
            self.ky_coeff = 0.0
        elif ph_constraint == "kx=0":
            self.domega_x = 0.0
            self.n_mode_x = 1
            self.x_grid_1d = np.array([0.0])
            self.kx_coeff = 0.0
...
class FFCavPhFPSocket(ForceField):
    ...
    def __init__(
        ...
        # a socket to the communication library is created or linked
        super(FFCavPhFPSocket, self).__init__(
            latency, name, pars, dopbc, active, threaded
        )
        if interface is None:
            self.socket = InterfaceSocket()
        ...
        self.ph = PhotonDriverFabryPerot(
            ...
            E0=E0,
            omega_c_cminv=omega_c_cminv,
            domega_x_cminv=domega_x_cminv,
            domega_y_cminv=domega_y_cminv,
            n_mode_x=n_mode_x,
            n_mode_y=n_mode_y,
            x_grid_1d=x_grid_1d,
            y_grid_1d=y_grid_1d,
...
class FFCavPh(ForceField):
    ...
    def __init__(
        ...
        if self.n_independent_bath > 1:
            print(
                "### Invoking independent baths approximatin for ab initio calculations ###"
            )
        ...
        self.init_nuclear_str = self.initialize_from_xyz(
            filename=self.input_xyz_filename
        )
        ...
        if self.n_qm_atom > 0 and self.n_qm_atom < self.nat_idb:
            ...
            if self.mm_charge_array.size + self.n_qm_atom == self.nat_idb:
                print(
                    "--- Number of QM atoms plus the MM charges equals to total atoms per bath ---"
                )
                print(
                    "--- Will append MM molecules to total dipole moment and dipole derivatives ---"
                )
            else:
                print(
                    "By default, setting MM charges as zero when interacting with cavity modes"
                )
        ...
        if self.qm_charge_array.size > 0:
            print(
                "#! QM atoms assigned with pre-defined partial charges, will not calculate dipole or dipder explicitly"
            )
            print(self.qm_charge_array)
            if self.do_qmmm and self.qm_charge_array.size != self.n_qm_atom:
                softexit.trigger(
                    "With QM/MM calculations, wrong size of qm_charge_array"
                )
            if not self.do_qmmm and self.qm_charge_array.size != self.nat_idb:
                softexit.trigger(
                    "With pure QM calculations, wrong size of qm_charge_array"
                )
        if self.name == "psi4":
            ...
            if self.n_independent_bath == 1:
                ...
                self.molec = psi4.geometry(
                    self.init_nuclear_str + "\n symmetry c1\n noreorient \n nocom"
                )
            elif self.n_independent_bath > 1:
                print(
                    "%d atoms in the original input xyz file %s"
                    % (self.nat, self.input_xyz_filename)
                )
                print(
                    "With %d independent baths, each subsystem contains the following %d atoms:"
                    % (self.n_independent_bath, self.nat_idb)
                )
                self.init_nuclear_str_idb = "\n".join(
                    self.init_nuclear_str.split("\n")[: self.nat_idb]
                )
            ...
            print(
                "Running semiclassical nuclear-electronic orbital (NEO) Ehrenfest dynamics"
            )
            # check if we have a template to create the qchem file
            print(
                "QChem jobs will be run from the following Q-Chem template: %s"
                % self.qchem_template
            )
            ...
            raise ValueError(
                "%s pythonic force field is unavailable currently" % self.name
            )
    ...
    def evaluate(self, r):
        ...
        if self.apply_photon:
            ...
            (
                ...
                dipole_z_tot,
                dipder_splitted,
            ) = self.calc_bare_nuclear_force_dipder(q)
            ...
            while self.error_flag == True:
                ...
                if count_retry >= 50:
                    softexit.trigger(
                        "Error always detected in obtaining gradients, try to shutdown simulation..."
                    )
            info(
                "mux = %.6f muy = %.6f muz = %.6f [units of a.u.]"
                % (dipole_x_tot, dipole_y_tot, dipole_z_tot),
                verbosity.medium,
            )
            ...
            e += (
                ...
                + e_int
                + 0.5
                * self.photons.coeff_self
                * (dipole_x_tot**2 + dipole_y_tot**2)
            )
    ...
    def calc_bare_nuclear_force(self, q):
        if self.name == "psi4" and self.n_independent_bath == 1:
            ...
            g, wfn2 = psi4.gradient(
                self.grad_method, ref_wfn=wfn, molecule=self.molec, return_wfn=True
            )
            force = -np.asarray(g)
            ...
            for idx_idb in range(self.n_independent_bath):
                q_sub = q[
                    int(idx_idb * self.nat_idb * 3) : int(
                        (idx_idb + 1) * self.nat_idb * 3
                    )
                ]
                # print("--- local q coordinate is\n", q_sub)
                self.molec.set_geometry(
                    psi4.core.Matrix.from_array(q_sub.reshape((-1, 3)))
                )
                E, wfn = psi4.energy(
                    self.grad_method, return_wfn=True, molecule=self.molec
                )
                force = -np.asarray(
                    psi4.gradient(self.grad_method, ref_wfn=wfn, molecule=self.molec)
                )
            ...
            for idx_idb in range(self.n_independent_bath):
                ...
                for idx in range(self.nat_idb):
                    local_str = "%s %.8f %.8f %.8f\n" % (
                        self.atom_label_lst[idx + idx_start],
                        q[(idx + idx_start) * 3],
                        q[(idx + idx_start) * 3 + 1],
                        q[(idx + idx_start) * 3 + 2],
                    )
            ...
            process = subprocess.Popen(
                bashCommand, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            process.communicate()
            ...
            print("energy is", E)
            print(
                "muex = %.6f muey = %.6f muez = %.6f [units of a.u.]"
                % (mue_info[0], mue_info[1], mue_info[2])
            )
            print(
                "munx = %.6f muny = %.6f munz = %.6f [units of a.u.]"
                % (mun_info[0], mun_info[1], mun_info[2])
            )
            return E, force
        ...
        if self.name == "psi4" and self.n_independent_bath == 1:
            ...
            for idx_idb in range(self.n_independent_bath):
                ...
                if self.qm_charge_array.size > 0:
                    ...
                    H, wfn2 = psi4.hessian(
                        self.grad_method, return_wfn=True, ref_wfn=wfn
                    )
                    dipder = wfn2.variable("SCF DIPOLE GRADIENT").np
                E_tot += E
                mux_tot += mux
            ...
            for idx_idb in range(self.n_independent_bath):
                ...
                try:
                    ...
                    os.rename(
                        "%s/energy.au" % IPI_DRIVER_TEMP,
                        "%s/energy.au.last_step" % IPI_DRIVER_TEMP,
                    )
                    os.rename(
                        "%s/egrad.au" % IPI_DRIVER_TEMP,
                        "%s/egrad.au.last_step" % IPI_DRIVER_TEMP,
                    )
                    os.rename(
                        "%s/dipole.debye" % IPI_DRIVER_TEMP,
                        "%s/dipole.debye.last_step" % IPI_DRIVER_TEMP,
                    )
                    os.rename(
                        "%s/dipder.au" % IPI_DRIVER_TEMP,
                        "%s/dipder.au.last_step" % IPI_DRIVER_TEMP,
                    )
                except:
                    None
