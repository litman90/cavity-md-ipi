...
class InputFFCavPhSocket(InputForceField):
    ...
    def fetch(self):
        ...
        return FFCavPhSocket(
            ...
            active=self.activelist.fetch(),
            threaded=self.threaded.fetch(),
            interface=InterfaceCavPhSocket(
                address=self.address.fetch(),
                port=self.port.fetch(),
                slots=self.slots.fetch(),
                mode=self.mode.fetch(),
...
class InputFFCavPhFPSocket(InputForceField):
    ...
    def fetch(self):
        ...
        return FFCavPhFPSocket(
            ...
            domega_x_cminv=self.domega_x_cminv.fetch(),
            domega_y_cminv=self.domega_y_cminv.fetch(),
            n_mode_x=self.n_mode_x.fetch(),
            n_mode_y=self.n_mode_y.fetch(),
            x_grid_1d=self.x_grid_1d.fetch(),
            y_grid_1d=self.y_grid_1d.fetch(),
            ph_constraint=self.ph_constraint.fetch(),
...
class InputFFCavPh(InputForceField):
    fields = {
        ...
        "numpy_memory": (
            InputValue,
            {"dtype": int, "default": 2, "help": "Memory for PSI4 numpy (in Gb)"},
        ),
        "nthread": (
            InputValue,
            {
    ...
    def fetch(self):
        ...
        return FFCavPh(
            input_xyz_filename=self.input_xyz_filename.fetch(),
