...
class dipole:
    def __init__(self):
        ...
        if self.run_photon:
            ...
            if self.have_incoming_pulse:
                ...
                print(
                    "## Adding a pulse at %d direction (0-x, 1-y, 2-z) for molecules ##"
                ...
                # pulse_atoms = [1, 2, 3]
                self.pulse_atoms = np.array(
                    data.get("pulse_atoms", [0, 1, 2]), dtype=np.int32
                )
                ...
                self.pulse_atoms_force_index = np.array(
                    [x * 3 + self.add_pulse_direction for x in self.pulse_atoms]
                )
                self.t = data.get("t0", 0.0)
                self.dt = data.get("dt", 0.5)
                print(
                    "## add initial pulse with E0 %.2E at time %.2f for molecules ##"
            ...
            if self.have_incoming_cw:
                ...
                print(
                    "## Adding a cw field at %d direction (0-x, 1-y, 2-z) for molecules ##"
                ...
                if type(self.cw_params[0]) is list:
                    ...
                    for idx in range(len(self.cw_params)):
                        ...
                        if self.cw_params[idx][2] == "RANDOM_PHASE":
                            ...
                            print(
                                "## Random phase is set as %.4f for cw No.%d ##"
                                % (self.cw_params[idx][2], idx)
                            )
                        print(
                ...
                self.cw_atoms = np.array(
                    data.get("cw_atoms", [0, 1, 2]), dtype=np.int32
                )
                ...
                # calculate the corresponding index in the force (correspond to the x axis)
                # atom 1 atom 2 atom 3
                self.cw_atoms_force_index = np.array(
                    [x * 3 + self.add_cw_direction for x in self.cw_atoms]
                )
                self.t = data.get("t0", 0.0)
                self.dt = data.get("dt", 0.5)
            else:
            ...
            if self.update_charge:
                print(
                    "## Remember, please use fix cavphipi for LAMMPS if updating charge ##"
                )
    ...
    def add_pulse(self, mf):
        if self.have_incoming_pulse:
            ...
            if (
                ...
                and self.t < self.pulse_params[4] + self.pulse_params[1] * 4.0
            ):
                self.set_charges()
    ...
    def calc_dipoles_tot_array(self):
        ...
        return np.reshape(
            np.array(
                [
                    self.calc_dipoles_x_tot(),
                    self.calc_dipoles_y_tot(),
                    self.calc_dipoles_z_tot(),
                ]
            ),
            (1, 3),
        )
    ...
    def calc_dipoles_x_which(self, index=0):
        ...
        self.set_charges()
        return np.sum(
            self.charges[index * 3 : index * 3 + 3]
            * self.pos[index * 3 : index * 3 + 3, 0]
        )
