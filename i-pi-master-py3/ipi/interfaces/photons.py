...
class photon:
    def __init__(self):
        ...
        if self.apply_photon:
            ...
            print(
                "set photon freq as %.3f cm-1" % (self.hartree_to_cm_minus1 * self.freq)
            )
...
class photons:
    def __init__(self):
        ...
        if self.apply_photon:
            ...
            print(
                "set fundamental photon freq as %.3f cm-1"
                % (self.hartree_to_cm_minus1 * self.freq)
            )
            ...
            self.freq_lst = self.freq * np.array(
                [n for n in range(1, self.nmodes + 1)] * 2
            )
            ...
            self.coeff_self = np.sum(
                self.E0_lst[0 : self.nmodes] ** 2
                / self.mass
                / self.freq_lst[0 : self.nmodes] ** 2
            )
            self.pot_coeff = 0.5 * self.mass * self.freq_lst**2
            self.pot_coeff2 = (
                self.mass
                * np.reshape(np.array([[x, x, x] for x in self.freq_lst]), -1) ** 2
            )
            ...
            if self.have_incoming_pulse:
                ...
                print(
                    "## Adding a pulse on photons at %d direction (0-x, 1-y, 2-z) ##"
                    % self.add_pulse_direction
                )
                # pulse_params = ["E0", "tau_FWHM", "omega", "phase", "t0"]
                self.pulse_params = data.get(
                    "pulse_params", [1.0, 10.0, 3550.0, 3.14, 10.0]
                )
                if type(self.pulse_params[2]) is list:
                    print(
                        "In fact, we add many gaussian pulses on photons at the same time..."
                    )
                    self.pulse_params[2] = np.array(
                        [x * 2.998e-5 * 2.0 * np.pi for x in self.pulse_params[2]]
                    )
                else:
                    self.pulse_params[2] *= (
                        2.998e-5 * 2.0 * np.pi
                ...
                self.transition_photon_charge = data.get(
                    "transition_photon_charge", 0.0
                )
                print(
                    "## add initial pulse on photons with E0 %.2E at time %.2f ##"
                    % (self.pulse_params[0], self.pulse_params[4])
                )
            else:
            ...
            if self.have_incoming_cw:
                ...
                print(
                    "## Adding a cw field on photons at %d direction (0-x, 1-y, 2-z) ##"
                    % self.add_cw_direction
                )
                ...
                if type(self.cw_params[0]) is list:
                    print(
                        "In fact, we add many cw waves on photons at the same time..."
                    )
                    self.have_many_cw = True
                    for idx in range(len(self.cw_params)):
                        self.cw_params[idx][1] *= 2.998e-5 * 2.0 * np.pi
                        print(
                            "## For cw No.%d, add initial cw with E0 %.2E at time %.2f ending at \
                            %.2f##"
                            % (
                                idx,
                                self.cw_params[idx][0],
                                self.cw_params[idx][3],
                                self.cw_params[idx][4],
                            )
                        )
                else:
                    ...
                    print(
                        "## add initial cw with E0 %.2E at time %.2f ending at \
                        %.2f##"
                        % (self.cw_params[0], self.cw_params[3], self.cw_params[4])
                    )
                self.t = data.get("t0", 0.0)
                self.dt = data.get("dt", 0.5)
                self.transition_photon_charge = data.get(
    ...
    def add_pulse(self, mf):
        if self.have_incoming_pulse:
            self.t += self.dt
            if (
                self.t > self.pulse_params[4]
                and self.t < self.pulse_params[4] + self.pulse_params[1] * 2.0
            ):
                t = self.t - self.pulse_params[4]  # - self.pulse_params[1]*4.0
                Ex = np.sum(
                    self.pulse_params[0]
                    * np.exp(-(t**2) / self.pulse_params[1] ** 2 * 2.0 * np.log(2.0))
                    * np.sin(self.pulse_params[2] * self.t + self.pulse_params[3])
                )
                if self.add_pulse_direction == 0:
                    mf[0 : self.nmodes * 3 : 3] += Ex * self.transition_photon_charge
                elif self.add_pulse_direction == 1:
        ...
        if self.have_incoming_cw:
            ...
            if not self.have_many_cw:
                ...
                if self.t > self.cw_params[3] and self.t < self.cw_params[4]:
                    Ex = self.cw_params[0] * np.cos(
                        self.cw_params[1] * self.t + self.cw_params[2]
                    )
                    if self.add_cw_direction == 0:
                        mf[0 : self.nmodes * 3 : 3] += (
                            Ex * self.transition_photon_charge
                        )
                    elif self.add_cw_direction == 1:
                        mf[self.nmodes * 3 + 1 :: 3] += (
                            Ex * self.transition_photon_charge
                        )
            else:
                if phase is None:
                    for cw_params in self.cw_params:
                        if cw_params[2] == "RANDOM_PHASE":
                            ...
                            print(
                                "Initialize the cw phase for photons as", cw_params[2]
                            )
                else:
                ...
                for cw_params in self.cw_params:
                    if self.t > cw_params[3] and self.t < cw_params[4]:
                        Ex += np.sum(
                            cw_params[0] * np.cos(cw_params[1] * self.t + cw_params[2])
                        )
                if self.add_cw_direction == 0:
                    mf[0 : self.nmodes * 3 : 3] += Ex * self.transition_photon_charge
                elif self.add_cw_direction == 1:
