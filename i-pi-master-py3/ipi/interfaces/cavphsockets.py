        ...
        while bpos < blen:
            ...
            try:
                ...
                if ntimeout > NTIMEOUT:
                    warning(
                        " @CavPhSOCKET:  Couldn't receive within %5d attempts. Time to give up!"
                        % (NTIMEOUT),
                        verbosity.low,
                    )
                    raise Disconnected()
...
class Driver(DriverSocket):
    ...
    def _getstatus(self):
        ...
        try:
            ...
            warning(
                " @CavPhSOCKET:   Other socket exception. Disconnecting client and trying to carry on.",
                verbosity.debug,
            )
            return Status.Disconnected
        ...
        if not len(reply) == HDRLEN:
            ...
            warning(
                " @CavPhSOCKET:    Unrecognized reply: " + str(reply), verbosity.low
            )
            return Status.Up
    ...
    def getforce(self):
        ...
        if self.status & Status.HasData:
            ...
            while True:
                try:
                    ...
                    warning(
                        " @CavPhSOCKET:   Timeout in getforce, trying again!",
                        verbosity.low,
                    )
                    continue
                except:
                    warning(
                        " @CavPhSOCKET:   Error while receiving message: %s" % (reply),
                        verbosity.low,
                    )
                    raise Disconnected()
                if reply == Message("forceready"):
                    ...
                    warning(
                        " @CavPhSOCKET:   Unexpected getforce reply: %s" % (reply),
                        verbosity.low,
                    )
                if reply == "":
                    raise Disconnected()
        ...
        if self.apply_photon:
            ...
            (
                ...
                dmudx,
                dmudy,
            ) = self.dipole.calc_dipole_x_y_and_derivatives()
            info(
                "mux = %.6f muy = %.6f muz = %.6f [units of a.u.]"
                % (dipole_x_tot, dipole_y_tot, self.dipole.calc_dipoles_z_tot()),
                verbosity.medium,
            )
    ...
    def dispatch(self, r):
        ...
        if not self.status & Status.Up:
            warning(
                " @CavPhSOCKET:   Inconsistent client state in dispatch thread! (I)",
                verbosity.low,
            )
            return
        ...
        if not (self.status & Status.Ready):
            warning(
                " @CavPhSOCKET:   Inconsistent client state in dispatch thread! (II)",
                verbosity.low,
            )
            return
        ...
        if not (self.status & Status.HasData):
            warning(
                " @CavPhSOCKET:   Inconsistent client state in dispatch thread! (III)",
                verbosity.low,
            )
            return
...
class InterfaceCavPhSocket(object):
    ...
    def close(self):
        ...
        try:
            ...
            info(
                " @CavPhSOCKET: Problem shutting down the server socket. Will just continue and hope for the best.",
                verbosity.low,
            )
        if self.mode == "unix":
    ...
    def pool_update(self):
        ...
        while keepsearch:
            ...
            if self.server in readable:
                ...
                info(
                    " @CavPhSOCKET:   Client asked for connection from "
                    + str(address)
                    + ". Now hand-shaking.",
                    verbosity.low,
                )
                ...
                if driver.status | Status.Up:
                    ...
                    info(
                        " @CavPhSOCKET:   Handshaking was successful. Added to the client list.",
                        verbosity.low,
                    )
                    ...
                    warning(
                        " @CavPhSOCKET:   Handshaking failed. Dropping connection.",
                        verbosity.low,
                    )
    ...
    def dispatch_free_client(self, fc, match_ids="any", send_threads=[]):
        ...
        for r in self.prlist[:]:
            ...
            info(
                " @CavPhSOCKET: %s Assigning [%5s] request id %4s to client with last-id %4s (% 3d/% 3d : %s)"
                % (
    ...
    def check_job_finished(self, r, c, ct):
        ...
        if (
            ...
            warning(
                " @CavPhSOCKET:  Timeout! request has been running for "
